...
export class PlancheComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.plancheService
      ...
      .subscribe({
        next: (res: HttpResponse<IPlanche[]>) => {
  ...
  trackId(_index: number, item: IPlanche): number {
    return item.id!;
  }

  delete(planche: IPlanche): void {
    const modalRef = this.modalService.open(PlancheDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onSuccess(data: IPlanche[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/planche'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
